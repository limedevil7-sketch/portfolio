...
const CONFIG = {
  ...
  profileImage: "assets/profile.jpg", // your real photo — swap this file (or path) to update everywhere at once

  // ---- Hero -------------------------------------------------------
  shortBio: "[SHORT BIO — one or two punchy sentences about who you are.]",

  // ---- About section ------------------------------------------------
  about: "[MY BIO — write a longer paragraph here about who you are, what you do, and what drives you. This text is fully editable right here in the CONFIG object.]",
  ...
  // ---- Socials ------------------------------------------------------
  // Leave a `url` empty ("") to show the card as "Coming soon".
  socials: {
    instagram: {
    ...
    whatsapp: {
      number: "201125316970", // digits only, country code first, no + or spaces
  ...
  website: {
    ...
    previewImage: "", // optional: put a screenshot path here (e.g. "assets/website-preview.jpg") to replace the placeholder preview
  },
};
...
(() => {
  ...
  /* -------------------------------------------------------------
     Build the list of social platforms from CONFIG
     ------------------------------------------------------------- */
  function buildSocialList() {
    ...
    if (s.discord) {
      list.push({
        ...
        // Discord doesn't expose a public profile link from a raw user ID alone;
        // this deep link opens the user's profile inside Discord (web or app)
        // when you're logged in, which is the best available option.
        url: `https://discord.com/users/${s.discord.userId}`,
      });
    }
    if (s.whatsapp && s.whatsapp.number) {
      list.push({
        ...
        url: `https://wa.me/${s.whatsapp.number}?text=${encodeURIComponent(s.whatsapp.message || "")}`,
      });
    }
    return list;
  }
  ...
  function renderSocialCards() {
    ...
    grid.innerHTML = items
      .map((item) => {
        ...
        return `
          <${tag} class="social-card${disabled ? " is-disabled" : ""}" ${attrs}
            style="--icon-bg:${colors.bg || ""}; --icon-fg:${colors.fg || ""}; --card-grad:${colors.grad || ""}"
            data-reveal-card>
            <span class="social-card__icon">${ICONS[item.key] || ""}</span>
            <span class="social-card__body">
              <span class="social-card__platform">${item.platform}</span>
              ${item.handle ? `<span class="social-card__handle">${item.handle}</span>` : ""}
              ${item.description ? `<span class="social-card__desc">${item.description}</span>` : ""}
            </span>
            ${disabled ? `<span class="social-card__badge">SOON</span>` : `<span class="social-card__arrow">${ICONS.arrow}</span>`}
          </${tag}>
        `;
      })
      .join("");
  ...
  function renderFooterIcons() {
    ...
    wrap.innerHTML = items
      .map(
        (item) =>
          `<a href="${item.url}" target="_blank" rel="noopener noreferrer" aria-label="${item.platform}">${ICONS[item.key] || ""}</a>`
      )
      .join("");
  }
  ...
  function renderWebsiteCard() {
    ...
    const previewInner = site.previewImage
      ? `<img src="${site.previewImage}" alt="${site.name} preview" class="project-card__img" />`
      : `
        <div class="project-card__placeholder">
          <span class="project-card__placeholder-glow"></span>
          <span class="project-card__placeholder-icon">${ICONS.globe}</span>
        </div>
      `;

    host.innerHTML = `
      <a class="project-card" href="${site.url}" target="_blank" rel="noopener noreferrer" data-reveal-card>
        ...
        <div class="project-card__body">
          <h3 class="project-card__title">${site.name}</h3>
          <p class="project-card__desc">${site.description || ""}</p>
          <span class="btn btn--primary project-card__btn">
            <span>Visit Website</span>
            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M7 17 17 7"/><path d="M7 7h10v10"/></svg>
          </span>
        </div>
      </a>
    `;
  ...
  function renderInterests() {
    ...
    if (!list) return;
    list.innerHTML = CONFIG.interests.map((i) => `<li>${i}</li>`).join("");
  }
  ...
  function hydrateText() {
    ...
    $$("[data-hero-name], [data-hero-name-2], [data-brand], [data-footer-name], [data-footer-name-2], [data-about-name]").forEach(
      (el) => (el.textContent = CONFIG.name)
    );
  ...
  function initLoader() {
    ...
    // Small minimum display time so it reads as an intentional beat,
    // not a flash — but never blocks real page rendering.
  ...
  /* -------------------------------------------------------------
     ...
     ------------------------------------------------------------- */
  function initReveal() {
    const targets = $$("[data-reveal], [data-reveal-card]");
  ...
  function initParticles() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      p.style.setProperty("--d", `${10 + Math.random() * 14}s`);
  ...
  /* -------------------------------------------------------------
     Desktop-only: cursor glow + custom cursor
     ------------------------------------------------------------- */
  function initDesktopCursor() {
    ...
    window.addEventListener("mouseleave", () => glow && glow.classList.remove("is-active"));

    const hoverables = "a, button, .social-card, .project-card, li";
  ...
  /* -------------------------------------------------------------
     Subtle tilt on the hero ID card (desktop only)
     ------------------------------------------------------------- */
